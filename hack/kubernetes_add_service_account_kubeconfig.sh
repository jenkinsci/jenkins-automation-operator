...
extract_ca_crt_from_secret() {
    ...
    kubectl get secret -o json $SECRET_NAME --namespace "${NAMESPACE}" | jq -r '.data["ca.crt"]' | base64 -D > "${TARGET_FOLDER}/ca.crt"
    printf "done"
}

get_user_token_from_secret() {
    echo -e -n "\\nGetting user token from secret..."
    USER_TOKEN=$(kubectl get secret -o json ${SECRET_NAME} --namespace "${NAMESPACE}" | jq -r '.data["token"]' | base64 -D)
    printf "done"
}
