...
func GetSecretData(k8sClient client.Client, secretName, namespace string) (data []byte, requeue bool, err error) {
	...
	if errors.IsNotFound(err) {
		logger.V(log.VDebug).Info(fmt.Sprintf("Secret %s in namespace %s not found\n", secretName, namespace))
		return data, true, err
	} else if statusError, isStatus := err.(*errors.StatusError); isStatus {
		logger.V(log.VDebug).Info(fmt.Sprintf("Error getting secret %s in namespace %s: %v\n", secret, namespace, statusError.ErrStatus.Message))
		return data, true, err
	} else if err != nil {
		return data, true, err
	} else {
		logger.V(log.VDebug).Info(fmt.Sprintf("Found secret %s in namespace %s\n", secretName, namespace))
...
func WriteDataToTempFile(data []byte) (filename string, requeue bool, err error) {
    ...
    logger.V(log.VDebug).Info(fmt.Sprintf("Created File: %s", tmpFile.Name()))
    
    if _, err := tmpFile.Write(data); err != nil {
		logger.V(log.VDebug).Info("Failed to write to temporary file")
        return filename, true, err
    }

    if err := tmpFile.Close(); err != nil {
		logger.V(log.VDebug).Info("Failed to close temporary file")
		return filename, true, err
	}
